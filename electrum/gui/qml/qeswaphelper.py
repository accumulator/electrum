...
class QESwapServerNPubListModel(QAbstractListModel):
    ...
    def initModel(self, items):
        ...
        self._services = [{
                ...
                'timestamp': util.age(x['timestamp']),
            }
            for x in items
        ]
        self.endInsertRows()
...
class QESwapHelper(AuthMixin, QObject, QtEventListener):
    ...
    def init_swap_manager(self):
        ...
        # try:
        #     asyncio.run(lnworker.swap_manager.get_pairs())
        ...
        # coro = lnworker.swap_manager.get_pairs()

        assert not swap_manager.is_server, 'running as swap server not supported'

        if swap_manager.use_nostr and self._wallet.wallet.config.SWAPSERVER_NPUB is None:  # TODO enable nostr
            self._logger.debug('use_nostr is enabled but swapserver npub still undefined')

        transport = swap_manager.create_transport()
        ...
        def query_task():
            with transport:
                ...
                if swap_manager.use_nostr and not swap_manager.is_initialized.is_set():
                    ...
                    self.availableSwapServers.initModel(recent_offers)
